  ...
  if grade in curriculums and 0 <= curriculum_id < len(curriculums[grade]): # 입력값 검증
    ...
    if request.method == 'POST':
      ...
      response = client.chat.completions.create(
        model="gpt-4o-mini",
        # messages=[
        #   {'role': "system", "content": f"당신은 영어 교사입니다. 다음 학생의 {curriculum_title}에 대해서 영어로 답변을 하는 사람입니다."},
        ...
        messages=[
          {'role': "system", "content":(
            ...
            f"학생이 지속적으로 한국말로 묻더라도, 당신은 절대로 한국말로 답해서는 안 됩니다."
          )},
          {"role":"user", "content": user_input}
        ],
        temperature=0.7
