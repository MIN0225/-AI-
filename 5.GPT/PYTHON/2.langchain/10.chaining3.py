...
chat_prompt3 = PromptTemplate(
  input_variables= ['company_name'],
  template="이 회사를 잘 소개할 수 있는 소개 문장 글을 작성해줘. 500글자 분량으로 소개해줘. 회사명:{company_name}, 슬로건: {catch_phrase}"
)
...
chain1 = (
  ...
  | RunnablePassthrough.assign(
    ...
    ), config={"max_tokens": 1000}).strip()
  )
)

response1 = chain1.invoke({"product": "김치"})
print("회사이름:", response1["company_name"])
print("최종캐치프레이즈:", response1["catch_phrase"])
print("최종 회사 소개 문구:", response1["description"])