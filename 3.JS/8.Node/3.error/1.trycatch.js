try {
  const result = somVariableName * 2;
} catch (error) {
  // 발생하는 모든 오류를 퉁쳐서 잡은 것...
...
try {
  ...
  if (error instanceof SyntaxError) {
    ...
    console.log("문법 외 다른 오류 발생", error.message);
  }
}

// 범위 에러 예시
try {
  let arr = new Array(-1); // 배열은 길이가 양수여야 함.
} catch (error) {
  if (error instanceof RangeError) {
    console.log("범위 오류 발생:", error.message);
  } else {
    console.log("레인지 에러 외 다른 오류:", error.message);
  }
}
